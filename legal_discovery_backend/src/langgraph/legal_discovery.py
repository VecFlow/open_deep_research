...
from .utils import (
    format_categories, 
    get_config_value, 
    search_documents_with_weaviate
)
...
async def generate_analysis_plan(state: LegalAnalysisState, config: RunnableConfig):
    ...
    # Search documents using Weaviate (renamed from Azure)
    source_docs = await search_documents_with_weaviate(query_list, configurable)
...
async def search_documents(state: CategoryState, config: RunnableConfig):
    ...
    2. Executes searches in Weaviate
    ...
    # Search documents using Weaviate
    source_docs = await search_documents_with_weaviate(query_list, configurable)
